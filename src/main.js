			...
			await (new Promise(resolve => {
				const waitNoteTimeout = setTimeout(() => {
					...
					if (isSingleServer) {
						...
						if (hasMultipleMainServers) console.log('WARNING: The bot will not function correctly until it\'s invited to *all* main servers and the inbox server.\n');
						else console.log('WARNING: The bot will not function correctly until it\'s invited to *both* the main server and the inbox server.\n');
					}
...
function waitForGuild(guildId) {
	if (bot.guilds.has(guildId)) return Promise.resolve();

	return new Promise(resolve => {
		bot.on('guildAvailable', guild => {
			if (guild.id === guildId) resolve();
		});
	});
}
...
function initStatus() {
	function applyStatus() {
		...
		}[config.statusType] || Eris.Constants.ActivityTypes.LISTENING;
		if (type === Eris.Constants.ActivityTypes.STREAMING) bot.editStatus(null, { name: config.status, type, url: config.statusUrl });
		else bot.editStatus(null, { name: config.status, type });
	}

	if (config.status == null || config.status === '' || config.status === 'none' || config.status === 'off') return;
...
function initBaseMessageHandlers() {
	...
	bot.on('messageCreate', async msg => {
		...
		if (!msg.author.bot && (msg.content.startsWith(config.prefix) || msg.content.startsWith(config.snippetPrefix))) {
			...
			// Only staff are allowed to reply
	...
	bot.on('messageCreate', async msg => {
		...
		// Ignore pins etc.
		if (msg.type !== Eris.Constants.MessageTypes.DEFAULT && msg.type !== Eris.Constants.MessageTypes.REPLY) return;
		if (await blocked.isBlocked(msg.author.id)) {
			// ignore silently
			if (config.blockedReply != null) channel.createMessage(config.blockedReply).catch(utils.noop);
	...
	bot.on('messageUpdate', async (msg, oldMessage) => {
		...
		if (threadMessage.isChat()) thread.updateChatMessageInLogs(msg);
	});
	/**
	...
	bot.on('messageDelete', async msg => {
		...
		if (threadMessage.isFromUser() && config.updateMessagesLive) bot.deleteMessage(thread.channel_id, threadMessage.inbox_message_id);
	...
	bot.on('messageCreate', async msg => {
		...
		if (mainGuilds.length === 1) content = `${staffMention}Bot mentioned in ${channel.mention} by ${userMentionStr}: "${msg.cleanContent}"\n\n<${messageLink}>`;
		else content = `${staffMention}Bot mentioned in ${channel.mention} (${channel.guild.name}) by ${userMentionStr}: "${msg.cleanContent}"\n\n<${messageLink}>`;
...
function initUpdateNotifications() {
	if (config.updateNotifications) updates.startVersionRefreshLoop();
}
...
async function loadAllPlugins() {
	...
	if (config.commandAliases) {
		for (const alias in config.commandAliases) commands.addAlias(config.commandAliases[alias], alias);
	}
