...
function initBaseMessageHandlers() {
  ...
  bot.on('messageCreate', async msg => {
    ...
    content = `${staffMention}Bot mentioned in ${msg.channel.mention} by <@${msg.author.id}>: "${msg.cleanContent}"\n\nhttps:\/\/discordapp.com\/channels\/${msg.channel.guild.id}\/$msg.channel.id}\/${msg.id}`;
    } else {
    content = `${staffMention}Bot mentioned in ${msg.channel.mention} (${msg.channel.guild.name}) by <@${msg.author.id}>: "${msg.cleanContent}"https:\/\/discordapp.com\/channels\/${msg.channel.guild.id}\/$msg.channel.id}\/${msg.id}`;
    }
