...
function initBaseMessageHandlers() {
	...
	bot.on("messageCreate", async msg => {
		...
		if (!msg.author.bot && (msg.content.startsWith(config.prefix) || msg.content.startsWith(config.snippetPrefix))) {
			...
			if (!utils.isStaff(msg.member))
				return; // Only staff are allowed to reply
	...
	bot.on("messageCreate", async msg => {
		...
		if (msg.author.bot)
			return;
		if (msg.type !== Eris.Constants.MessageTypes.DEFAULT && msg.type !== Eris.Constants.MessageTypes.REPLY)
		...
		messageQueue.add(async () => {
			...
			if (createNewThread) {
				...
				if (config.ignoreAccidentalThreads && msg.content && ACCIDENTAL_THREAD_MESSAGES.includes(msg.content.trim().toLowerCase()))
					return;
	...
	bot.on("messageCreate", async msg => {
		...
		if (! await utils.messageIsOnMainServer(bot, msg))
			return;
		...
		if (await utils.messageIsOnInboxServer(bot, msg)) {
			...
			if (inboxMember && utils.isStaff(inboxMember))
				return;
		}
