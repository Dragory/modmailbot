...
class ThreadMessage {
	constructor(props) {
		...
		if (props.attachments) {
			if (typeof props.attachments === 'string') this.attachments = JSON.parse(props.attachments);
		}
		else {
			this.attachments = [];
		}
		...
		if (props.small_attachments) {
			if (typeof props.small_attachments === 'string') this.small_attachments = JSON.parse(props.small_attachments);
		}
		else {
			this.small_attachments = [];
		}
		if (props.metadata) {
			if (typeof props.metadata === 'string') this.metadata = JSON.parse(props.metadata);
		}
	}
