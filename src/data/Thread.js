...
class Thread {
  ...
  async replyToUser(moderator, text, replyAttachments = [], isAnonymous = false, messageReference = null) {
    let moderatorName = config.useNicknames && moderator.nick ? moderator.nick : moderator.user.globalName;
  ...
  async receiveUserReply(msg, skipAlert = false) {
    ...
    let threadMessage = new ThreadMessage({
      ...
      user_name: msg.author.globalName,
      body: messageContent,
      is_anonymous: 0,
  ...
  async scheduleClose(time, user, silent) {
    await knex("threads")
      ...
      .update({
        ...
        scheduled_close_name: user.globalName,
  ...
  async scheduleSuspend(time, user) {
    await knex("threads")
      ...
      .update({
        ...
        scheduled_suspend_name: user.globalName
      });
  }
