...
module.exports = ({ bot }) => {
	...
	if (config.notifyOnMainServerJoin) {
		bot.on('guildMemberAdd', async (guild, member) => {
			...
			if (thread != null) await thread.postSystemMessage(`***The user joined the ${guild.name} server.***`);
		});
	}
	...
	if (config.notifyOnMainServerLeave) {
		bot.on('guildMemberRemove', async (guild, member) => {
			...
			setTimeout(async () => {
				const thread = await threads.findOpenThreadByUserId(member.id);
				if (thread != null) {
					if (leaveIgnoreIDs.includes(member.id)) leaveIgnoreIDs.splice(leaveIgnoreIDs.indexOf(member.id), 1);
					else await thread.postSystemMessage(`***The user left the ${guild.name} server.***`);
				}
			}, 2 * 1000);
		});
	}
	...
	if (config.notifyOnMainServerLeave) {
		bot.on('guildBanAdd', async (guild, user) => {
			...
			if (thread != null) {
				await thread.postSystemMessage(`***The user was banned from the ${guild.name} server.***`);
	...
	if (config.notifyOnMainServerJoin) {
		bot.on('guildBanRemove', async (guild, user) => {
			...
			const thread = await threads.findOpenThreadByUserId(user.id);
			if (thread != null) await thread.postSystemMessage(`***The user was unbanned from the ${guild.name} server.***`);
		});
	}
};
