...
module.exports = ({ bot, knex, config, commands }) => {
  async function getMessagesAmounts(thread) {
    ...
    return [
      `**${fromUserMessages.length}** message${
        fromUserMessages.length !== 1 ? "s" : ""
      } from the user`,
      `, **${toUserMessages.length}** message${
        toUserMessages.length !== 1 ? "s" : ""
      } to the user`,
      ` and **${chatMessages.length}** internal chat message${
  ...
  async function applyScheduledCloses() {
    ...
    for (const thread of threadsToBeClosed) {
      ...
      await sendCloseNotification(
        thread,
        `Modmail thread #${thread.thread_number} with ${thread.user_name} (${thread.user_id}) was closed as scheduled by ${thread.scheduled_close_name}`
      );
    }
  }
    ...
    async (msg, args) => {
      ...
      if (msg.channel instanceof Eris.PrivateChannel) {
        ...
        if (delayStringArg) {
          ...
          let response;
          if (silentClose) {
            response = `Thread is now scheduled to be closed silently in ${utils.humanizeDelay(
              delay
            )}. Use \`${config.prefix}close cancel\` to cancel.`;
          } else {
            response = `Thread is now scheduled to be closed in ${utils.humanizeDelay(
              delay
            )}. Use \`${config.prefix}close cancel\` to cancel.`;
          }
