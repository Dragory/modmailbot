...
module.exports = ({ bot, config, commands }) => {
	...
	async function sendCloseNotification(thread, body) {
		...
		if (logUrl) {
			utils.postLog(utils.trimAll(`${body} Logs: ${logUrl}`));
	...
	commands.addGlobalCommand('close', '[opts...]', async (msg, args) => {
		...
		if (msg.channel instanceof Eris.PrivateChannel) {
			...
			if (args.silent || opts.includes('silent') || opts.includes('s')) silentClose = true;
			...
			if (delayStringArg) {
				...
				let response;
				if (silentClose) response = `Thread is now scheduled to be closed silently in ${utils.humanizeDelay(delay)}. Use \`${config.prefix}close cancel\` to cancel.`;
				else response = `Thread is now scheduled to be closed in ${utils.humanizeDelay(delay)}. Use \`${config.prefix}close cancel\` to cancel.`;
