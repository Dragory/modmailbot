...
module.exports = ({ bot, config, commands, hooks }) => {
	...
	const logsCmd = async (msg, args, thread) => {
		const userId = args.userId || (thread && thread.user_id);
		if (!userId) return;
		...
		const page = Math.max(Math.min(inputPage ? parseInt(inputPage, 10) : 1, maxPage), 1);
	...
	const logCmd = async (msg, args, _thread) => {
		...
		if (customResponse && (customResponse.content || customResponse.file)) channel.createMessage(customResponse.content, customResponse.file);
